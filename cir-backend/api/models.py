...
from .constants import CardinalDirection, CrisisCategory, DamageSeverity
...
class InfrastructureLocation(models.Model):
    ...
    infrastructure_location_name = models.CharField(max_length=255, blank=True, null=True, help_text="Name of the infrastructure location (e.g., 'Main Street Bridge', 'Downtown Intersection', etc.)")
    ...
    submit_location_description = models.CharField(max_length=255, blank=True, null=True, help_text="Free text description of the submit location")
    submit_location_name = models.CharField(max_length=255, blank=True, null=True, help_text="Name of the submit location (e.g., 'Main Street Bridge', 'Downtown Intersection', etc.)")

    submit_location_to_infrastructure_distance = models.FloatField(blank=True, null=True, help_text="Distance in meters from the submit location to the infrastructure location.")
    
    submit_location_to_infrastructure_direction_degrees = models.FloatField(blank=True, null=True, help_text="Direction from the submit location to the infrastructure location in degrees (0-360, where 0/360 is north, 90 is east, 180 is south, and 270 is west).")
    ...
    submit_location_to_infrastructure_direction_description = models.CharField(max_length=255, blank=True, null=True, help_text="Free text description of the direction from the submit location to the infrastructure location")
...
class ImpactReport(models.Model):
    ...
    """
    crisis = models.ForeignKey(Crisis, on_delete=models.SET_NULL, blank=True, null=True, help_text="The crisis event that this impact report is related to. This can be left blank if the report is not linked to a specific crisis event.")
    photos = models.ManyToManyField(Photo)
    description = models.TextField(max_length=255, blank=True, null=True)
    infrastructure_name = models.CharField(max_length=255, blank=True, null=True)
    infrastructure_type = models.CharField(max_length=255, blank=True, null=True,
                                           help_text="e.g., bridge, road, building, etc."
                                           )
    
    infrastructure_description = models.TextField(max_length=255, blank=True, null=True)
    
    damage_severity = models.CharField(max_length=255, choices=DamageSeverity.choices, blank=True, null=True)
    ...
    debris_description = models.TextField(max_length=255, blank=True, null=True, help_text="Description of the debris, if present.")
...
class Question(models.Model):
    ...
    choice_options = models.JSONField(blank=True, null=True, help_text="A JSON array of multiple choice options for the question, if applicable. For example: ['Option 1', 'Option 2', 'Option 3']. If the question is not multiple choice, this can be left blank.")
...
class NatureOfCrisisQuestionAnswer(models.Model):
    question = models.ForeignKey(NatureOfCrisisQuestion, on_delete=models.CASCADE)
...
class CrisisQuestion(Question):
    crisis = models.ForeignKey(Crisis, on_delete=models.CASCADE, blank=True, null=True, help_text="The crisis event that this question is related to. This can be left blank if the question is not linked to a specific crisis event.")
